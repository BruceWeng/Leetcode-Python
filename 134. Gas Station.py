...
"""
Algorithm:
1. If car starts at A and can not reach B, any stations between A and B can not reach B
...
def canCompleteCircuit(gas, cost):
    total = 0 # the total sum of all gas[i] - cost[i]
    result = 0
    tank = 0 # keep track of gas in the tank

    for i in range(len(gas)):
        tank += gas[i] - cost[i]
        total += gas[i] - cost[i]

        # If tank < 0, then we know we can't move from any one of 0...i-1 to i
        # so set result to i+1 and tank to 0
        if tank < 0:
            result = i + 1
            tank = 0
